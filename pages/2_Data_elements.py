...
df1 = pd.DataFrame(np.random.randn(5,10), columns=['col_%d'% (i+1) for i in range(10)])
...
st.markdown('num_rows=dynamic => 사용자가 행을 추가하거나 삭제할 수 있음')
st.data_editor(df3_part1, num_rows='dynamic')

st.markdown('disabled=[col명,...] => 지정한 열 내용 수정불가')
