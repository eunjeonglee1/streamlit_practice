...
st.dataframe(df)
st.markdown('column(label=, help=, width=) 사용')
...
st.markdown('column(label=, help=, width=, disabled=) 사용')
...
st.markdown('text_column(default=) 사용 => 행을 새로 추가하면 default에 쓴 내용이 자동적으로 나타남')
st.data_editor(df,column_config={
...
st.markdown('text_column(max_chars=) 사용 => 글자수 제한')
st.data_editor(df,column_config={
    'command':st.column_config.TextColumn(
        label='Streamlit Commands',
        help='Streamlit **widget** Commands',
...
st.markdown('text_column(validate=) 사용 => 문자 제한(유효성 검사)')
st.data_editor(df,column_config={
    'command':st.column_config.TextColumn(
        label='Streamlit Commands',
        help='Streamlit **widget** Commands',
        default='st.', max_chars=20,validate='^st\.[a-z_]+$')}, num_rows='dynamic')
...
df3 = pd.DataFrame({'meeting_date' : [datetime(2024,2,5,12,30),
...
df4 = pd.DataFrame({'meeting_date' : [date(2024,2,5),
                    date(2024,2,7),
                    date(2024,3,5),
                    date(2024,3,25),
                    date(2024,4,5)]
...
st.dataframe(df4)
st.data_editor(df4, column_config={'meeting_date':st.column_config.DateColumn(
...
df5 = pd.DataFrame({'meeting_date' : [time(12,30),
                    time(2,30),
                    time(10,00),
                    time(11,00),
                    time(10,30)]
...
st.data_editor(df5, column_config={'meeting_date':st.column_config.TimeColumn(
...
st.dataframe(df6)
st.data_editor(df6,column_config={'score':st.column_config.ListColumn(
    width='medium'
)})
...
df7 = pd.DataFrame({'site':['naver','daum','google'],
                    'url':['https://www.naver.com','https://www.daum.net','https://www.google.com']})

st.data_editor(df7,column_config={'url':st.column_config.LinkColumn(
...
st.dataframe(df8,column_config={'score2':st.column_config.LineChartColumn(y_min=0,y_max=200),
'score3':st.column_config.AreaChartColumn(y_min=0,y_max=100),
'score':st.column_config.BarChartColumn(y_min=0,y_max=100)})
...
st.dataframe(df9,column_config={'image':st.column_config.ImageColumn()})
...
st.data_editor(df10, column_config={'sales':st.column_config.ProgressColumn(
