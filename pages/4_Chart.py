...
col1.metric(label='온도', value='22℃', delta='-1.2℃', delta_color='inverse')
col2.metric(label='체감온도', value='23℃', delta='1.1℃', delta_color='inverse')

st.divider()

col1, col2 = st.columns(2)
col1.metric(label='온도', value='22℃', delta='-1.2℃', delta_color='normal')
col2.metric(label='체감온도', value='23℃', delta='1.1℃', delta_color='off')
...
st.map(df_loc, size=500, color='#00ff00', zoom=14)
...
map = folium.Map(location=[37.574,126.973], zoom_start=9)
mc = MarkerCluster().add_to(map)

for gu,pos in loc.items() :
        folium.Marker(location=pos, popup=folium.Popup(gu, max_width=200)).add_to(mc)

st_folium(map, width=800, height=600)

st.divider()

st.markdown(':pushpin: :red[**columns를 이용해서 지도를 양쪽으로 그리는 방법**]')
...
map2 = folium.Map(location=[37.566,126.9782], zoom_start=8)
folium.GeoJson(geo_gg).add_to(map2)
folium.Choropleth(geo_data=geo_gg,
                  data=df_gg[2007],
                  columns=[df_gg.index, df_gg[2007]],
                  key_on='feature.properties.name').add_to(map2)
st_folium(map2,width=600, height=400)